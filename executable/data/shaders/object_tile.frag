...
void FragmentMain()
{
    ...
    float fIntensity  = dot(normalize(v_v3Normal2), vec3(0.0,0.0,1.0)) * 0.5 + 0.5;
          fIntensity *= dot(normalize(v_v3Relative), v3Camera);
          fIntensity *= min(inversesqrt(coreLengthSq(v_v3Relative)) * 50.0, 1.0);

    gl_FragColor = vec4(mix(u_v4Color.rgb * mix(1.0, 0.45, v1Dark), mix(u_v4Color.rgb * 0.45, vec3(1.0), v3Number), v1Alpha) * fIntensity, u_v4Color.a);
}